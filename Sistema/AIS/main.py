...
def consultar_dados():
    ...
    # Resultado da pagina sendo carregada a primeira vez, sem perceber o post
    return render_template_string('''
...
def graficos():
    ...
    fig1.update_layout(title='Evolução da Inadimplencia', xaxis_title='Mês', yaxis_title='%', template='plotly_dark')

    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=selic_df['mes'], y=selic_df['selic_diaria'], mode='lines+markers', name='SELIC'))
    fig2.update_layout(title='Média mensal da SELIC', xaxis_title='Mês', yaxis_title='%', template='plotly_dark')
